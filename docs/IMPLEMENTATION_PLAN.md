...
One terminal error middleware maps typed errors to `{ error: { code, message } }` + status, so
...
│     │  └─ likes.controller.ts
│     ├─ services/
│     │  ├─ unsplash.service.ts  fetch Unsplash, normalize to Photo, typed errors
│     │  ├─ photos.service.ts    page from unsplash + liked set from repo → merge
│     │  └─ likes.service.ts     validate + set/unset via repo
│     ├─ db/
│     │  ├─ connection.ts        better-sqlite3 singleton + schema on boot
│     │  └─ likes.repo.ts        the only place SQL lives
│     ├─ middleware/
│     │  └─ errorHandler.ts      typed errors → { error: { code, message } } + status
│     ├─ errors/
│     │  └─ AppError.ts          AppError + subclasses; ErrorCode enum
│     └─ types/
...
- `GET /api/health` → `200 { ok: true }` — lets me confirm the dev proxy works before any
  real endpoint exists.
- `GET /api/photos?page=1&per_page=8` — `page` ≥1, `per_page` 1–30; bad params →
  `400 VALIDATION_ERROR`. → `{ page, perPage, items: Photo[] }`. "Has more" is inferred from
  `items.length === perPage` (Unsplash has no total usable as a cursor).
- `PUT /api/photos/:id/like` → `{ id, liked: true }` · `DELETE /api/photos/:id/like` →
  ...
  races are safe.
- Error codes: `400 VALIDATION_ERROR`, `404 NOT_FOUND`, `429 UPSTREAM_RATE_LIMITED`
  (Unsplash 403/429 — the demo-tier 50 req/hr case), `502 UPSTREAM_ERROR`, `500 INTERNAL`.
  The client parses the envelope and branches the error state on `code`.
...
- **No `any`.** TypeScript runs in `strict` mode and `any` is banned. External data (Unsplash
  responses, request params) comes in as `unknown` and is narrowed with a type guard before
  use. Enforced by `tsc` + review.
...
- **Enums / const objects over loose strings.** Anything with a fixed set of values is an enum
  or a `const` object — error codes, API error codes, query-key roots, named breakpoints.
  No bare string literals for these scattered around the code.
...
| #   | Outcome                                                                                                                    | Commit                                                                          |
| --- | -------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| 0   | Monorepo scaffold: layered Express+TS (env validation, `/api/health`) + Vite/React/TS/Tailwind/Query, dev proxy, both boot | `chore: scaffold layered express server and vite client with dev proxy`         |
